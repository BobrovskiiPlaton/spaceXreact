...
function Map(props){
    ...
    const showMap = (launchpads) => {
        if (!launchpads || !mapRef.current || !projectionRef.current) return;
        mapRef.current.selectAll("circle").remove();
    ...
    useEffect(() => {
        if (props.launchpads && mapRef.current && projectionRef.current) {
    ...
    useEffect(()=> { 
        ...
        mapRef.current = g.append("g").attr("class", "launchpads");
